...
const TableCoin = ({ size , infos}: { size: string , infos: Infos}) => {
  ...
  const handleClick = () => {
    if (owned && equipped) {
      axios.post(`http://${ipAdress}:8080/shop/table/disable`, {
        user_id: user?.id,
        color: infos.color
      }).then(
        (res) => {
      ...
      axios.post(`http://${ipAdress}:8080/shop/table/enable`, {
        user_id: user?.id,
        color: infos.color
      }).then(
        (res) => {
      ...
      axios.post(`http://${ipAdress}:8080/shop/table`, {
        image: infos?.image,
        color: infos?.color,
        user_id: user?.id,
        price : infos?.price
